...
class AliyunBilling:
    ...
    def __init__(self, access_id: Optional[str] = None, access_key: Optional[str] = None,
                 region: str = "cn-hangzhou", account_id: str = ""):
        """
        初始化阿里云账单客户端
    ...
    def _create_client(self, access_key_id: Optional[str] = None,
                       access_key_secret: Optional[str] = None) -> BssOpenApi20171214Client:
        """
        创建阿里云BSS OpenAPI客户端
    ...
    def get_account_balance_amount(self) -> float:
        ...
        if response.body and response.body.data:
            # 阿里云返回的余额单位为元
            return float(response.body.data.available_amount or 0.0)
        return 0.0
